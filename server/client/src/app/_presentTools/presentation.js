...
class Presentation {
  ...
  dashboard(id, fData){
    ...
    function segColor(c){ return {True:"#807dba", False:"#e08214"}[c]; }

    // compute total for each state.
    fData.forEach(function(d){d.total=d.freq.True+d.freq.False;});
    ...
    function histoGram(fD){
      ...
      bars.append("rect")
        ...
        .attr('fill',barColor)
        .on("mouseover",mouseover)// mouseover is defined beTrue.
        .on("mouseout",mouseout);// mouseout is defined beTrue.
    ...
    let tF = ['True','False'].map(function(d){
  ...
  dashboard2(id, Data) {
    ...
    for (let i=0; i< array[0].length ; i++)
    {
      data[0].result.push(
        {
          axis: array[0][i].questionType,
          value: array[0][i].totalRight / (array[0][i].totalRight + array[0][i].totalWrong)
        }
      )
    }
    ...
    function handleMap2() {
      return data.map(function(d) {
        return {
          axes: d.result.map(function(axis) {
